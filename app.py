...
Sistema de Horas de Bienestar"""
            ...
            if not app.config.get('TWILIO_ACCOUNT_SID') or not app.config.get('TWILIO_AUTH_TOKEN'):
                # SMS no configurado - rechazar la solicitud
                logging.error('Twilio no configurado - No se puede enviar SMS')
                flash('El sistema no está configurado para enviar SMS. Contacta al administrador.', 'danger')
                write_audit('forgot_password_twilio_not_configured', 'user', usuario.id, 
        ...
        if success:
            # Mostrar página de confirmación (sin mostrar código)
            return render_template('forgot_password_sent.html', 
                                 delivery_info=delivery_info, 
                                 correo=correo,
                                 show_code=False)
        # Si llegamos aquí, hubo un error (ya mostrado con flash)
        return render_template('forgot_password.html')
